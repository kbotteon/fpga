...
You can start a VNC session as follows:
. Open your Codespaces built from this repository in VSCode for source editing
. From the command line, `vncserver -geometry 2560x1440 :1` or whatever resolution you prefer
. VSCode will automatically create a port forward to 5901
. Point a VNC viewer to `localhost:5901` and connect

=== Docker

The underlying Dockerfile for the development container is described in `.devcontainer/Dockerfile` and is build automatically when a Codespace is created. You can also retrive it from the GitHub Container Registry, where it is located at:

----
ghcr.io/kbotteon/fpga-tools:latest
----

To work locally or debug builds, you can build, run, and attach:
...
The port forward is for VNC. It assumes you use display `:1` when launching. To stop the image:

----
docker stop fpga_tools_1
docker rm fpga_tools_1
----

You must remove and recreate the image to change port mappings. Otherwise, skip the remove step and reattach with:

----
docker restart fpga_tools_1
docker exec -it fpga_tools_1 /bin/bash
----
