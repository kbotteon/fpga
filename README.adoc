...
The underlying Dockerfile for the development container is described in `.devcontainer/Dockerfile` and is build automatically when a Codespace is created. To use it locally, or to debug, you can build it yourself:
...
You must remove and recreate the image to change port mappings. Otherwise, skip the remove step and `docker restart fpga_tools_1` to reattach.
